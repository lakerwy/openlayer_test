  ...
  measureLineSpace(viewer) {
    ...
    handler.setInputAction(function(movement) {
      ...
      floatingPoint = viewer.entities.add({
        ...
        label: {
          ...
          pixelOffset: new Cartesian2(20, -35)
        }
      });
    ...
    handler.setInputAction(function(movement) {
      ...
      viewer.entities.add({
        ...
        label: {
          ...
          //outlineWidth: 2,
          verticalOrigin: VerticalOrigin.BOTTOM,
          pixelOffset: new Cartesian2(20, -10),
       ...
       * 鼠标点击事件
       */
        let lablehandler = new ScreenSpaceEventHandler(viewer.scene.canvas);
        lablehandler.setInputAction(function(click) {
          var pick = viewer.scene.pick(click.position);
          //选中某模型   pick选中的对象
          if (pick && pick.id && pick.id.name == "清除") {
            let count = viewer.entities._entities._array.length;
            if (count > 0) {
              for (let i = 0; i < count; i++) {
                if (viewer.entities._entities._array[i].name == pick.id.id) {
                  viewer.entities.remove(viewer.entities._entities._array[i]);
                  count = viewer.entities._entities._array.length;
                  i = -1;
                }
              }
            }
    ...
    var PolyLinePrimitive = (function() {
      ...
      _.prototype._init = function() {
        ...
        that.line = viewer.entities.add(this.options);
      };
    ...
    handler.setInputAction(function(movement) {
      ...
      lablehandler.setInputAction(function(click) {
        ...
        if (pick && pick.id && pick.id.name == "清除") {
          ...
          viewer.entities.remove(pick.id);
          //viewer.entities.removeById(pick.id);
        }
      }, ScreenSpaceEventType.LEFT_CLICK);
    }, ScreenSpaceEventType.RIGHT_CLICK);
